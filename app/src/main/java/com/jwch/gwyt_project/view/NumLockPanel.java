...
public class NumLockPanel extends LinearLayout {
    ...
    private ArrayList<CircleImageView> mcircleList;
    ...
    public NumLockPanel(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mPanelColor = Color.WHITE; //颜色代码可采用Color.parse("#000000");
    ...
    public void resetResult(){
        ...
        if (CommonUtil.matchList(mResultIvList)) {
            for (int i = 0; i <mResultIvList.size() ; i++) {
                CircleImageView item = mResultIvList.get(i);
                item.setStrokeCircle();
            }
        }
    }
    ...
    public void resetItem(){
        ...
        if (CommonUtil.matchList(mcircleList)) {
            for (int i = 0; i <mcircleList.size() ; i++) {
                CircleImageView item = mcircleList.get(i);
                item.setStrokeCircle();
            }
        }

        if (CommonUtil.matchList(mtvList)) {
            for (int i = 0; i <mtvList.size() ; i++) {
                TextView item = mtvList.get(i);
                item.setTextColor(mPanelColor);
            }
        }

    }

    public void isEnableClick(boolean enable){
        enableClick = enable;
    }
