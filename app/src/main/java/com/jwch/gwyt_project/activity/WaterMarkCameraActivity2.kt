...
import com.jwch.gwyt_project.util.*
...
class WaterMarkCameraActivity2 : FullScreenActivity<PageWatermarkCamera2Binding>(), View.OnTouchListener {
    ...
    lateinit var orientUtil: OrientUtil
    ...
    override fun initView() {
        ...
        vb.stvPosition.setOnTouchListener(this)
        dynamicCameraPreview()//设置拍摄画面比例
        initLocation()//定位信息
        orientUtil = OrientUtil(this){ direction,directionTxt->
            vb.tvOrient.text = "${directionTxt} ${direction}°"
        }
...
//            lp.height = screenHeight
            lp.width = (screenHeight * CameraPreview.SCALE_4_3).toInt()
            is16_9 = false

            vb.preview.layoutParams.width = lp.width
        } else {
//            竖屏
...
//        }
        ...
        mCamera!!.takePicture(null, null, null, { data: ByteArray?, camera1: Camera? ->
            ...
            bitmapManager!!.getPhoto(
                mFile, data, mCameraId, takePhotoOrientation, watermarkBitmap, cameraPreview!!.parameters
    ...
    private fun switchFlash() {
        ...
        vb.switchFlash.setImageResource(if (isFlashing) R.drawable.vector_drawable_canera_lighting_on else R.drawable.vector_drawable_canera_lighting_off)
        try {
            val parameters = mCamera!!.parameters
            parameters.flashMode = if (isFlashing) Parameters.FLASH_MODE_TORCH else Parameters.FLASH_MODE_OFF
    ...
    private fun cancleSavePhoto() {
        ...
        vb.switchFlash.visibility = View.VISIBLE
...
//        vb.stvRemark.show()
    ...
    var onBitmapCompleteListener: BitmapManager.OnBitmapCompleteListener = BitmapManager.OnBitmapCompleteListener { bitmap ->
        newBitmap = bitmap
    ...
    private fun initOrientate() {
        if (mOrientationEventListener == null) {
            mOrientationEventListener = object : OrientationEventListener(context, SensorManager.SENSOR_DELAY_UI) {
...
//                        System.out.println("takePhotoOrientation------>"+takePhotoOrientation);
                    ...
                    //预览方向设置
                    orientation = if (orientation > 30 && orientation < 60) { // 动画0度与接口360度相反,增加下限抵消0度影响
                        0
...
//                            vb.llPhotoMessage, "Rotation", oldOrientation.toFloat(), orientation.toFloat()
//                        ).setDuration(200)
...
//                            vb.llPhotoMessage.pivotY = -(screenWidth / 2 - photoMessageHeight).toFloat()
...
//                                vb.llPhotoMessage, "translationY", -(cameraPreview!!.height - screenWidth).toFloat(), 0F
//                            )
//                            translationY.duration = 0
//                            translationY.start()
//                        } else if (orientation == 90 || orientation == 180) {
//                            val translationY: ObjectAnimator = ObjectAnimator.ofFloat(
//                                vb.llPhotoMessage, "translationY", 0F, -(cameraPreview!!.height - screenWidth).toFloat()
//                            )
//                            translationY.duration = 0
//                            translationY.start()
//                        }
//                        vb.llPhotoMessage.clearAnimation()
    ...
    private val autoFocusCallback = AutoFocusCallback { success, camera ->
        isFoucing = false
