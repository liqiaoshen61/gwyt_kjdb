...
import com.jwch.gwyt_project.databinding.ViewTextLineCompat2Binding
...
class TextLineCompat2 : LinearLayout, DialogItemClickListener {

    val layoutId: Int = R.layout.view_text_line_compat2
    var vb: ViewTextLineCompat2Binding? = null
    fun initView(context: Context?) {
        context?.let {
            ...
            vb = ViewTextLineCompat2Binding.bind(contentView)
...
//    lateinit var latlngList: MutableList<LatLng>
    ...
    var showValueBorder2 = false //是否展示value的外框
    var showkeyView = true  //是否展示key
...
//            "控件类型：$viewType".printMsg()
            ...
            showValueBorder2 = getBoolean(R.styleable.TextLineCompat_showValueBorder2, false)
            showkeyView = getBoolean(R.styleable.TextLineCompat_showKeyView, true)
...
//            "key  $key   value  $value   hint  $hint  isreadonly  $isReadOnly".printMsg()
            ...
            keyColor = getColor(
                R.styleable.TextLineCompat_key_color,
                ContextCompat.getColor(context, R.color.key_form_color)
            )
            valueColor = getColor(
                R.styleable.TextLineCompat_value_color,
                ContextCompat.getColor(context, R.color.value_form_color)
            )
            ...
            keyWeight = getFloat(R.styleable.TextLineCompat_key_weight, 4f)
...
//            "strTimeFormatType  $strTimeFormatType   maxTime $defTime  miniTime $miniTime   defTime  $defTime".printMsg()
    ...
    fun initViewData() {
        ...
        vb?.apply {
            ...
            if (showValueBorder2) {
                rlValue.setBackgroundResource(R.drawable.bg_hollow_black2_border6)
            } else {
                rlValue.setBackgroundResource(R.color.transColor)
            }
            ...
            if (horizontal_padding > 0) {
                llTextline.setPadding(
                    horizontal_padding.toInt(),
                    llTextline.paddingTop,
                    horizontal_padding.toInt(),
                    llTextline.bottom
                )
            }
            if (vertical_padding > 0) {
                llTextline.setPadding(
                    llTextline.paddingLeft,
                    vertical_padding.toInt(),
                    llTextline.paddingRight,
                    vertical_padding.toInt()
                )
            }
...
//                    }
    ...
    private fun showTimeDialog(
        ...
        val build = CardDatePickerDialog.builder(mContext).setTitle(dialogTitle)
            .setDisplayType(timeformatList)
